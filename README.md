...
Spring Boot in `backend-spring/` is the current canonical backend for the project. The old Fastify implementation remains in `backend/` as an archived legacy implementation for history, rollback drills, and parity audits only. Do not add new backend functionality to Fastify; active backend work targets Spring.

## Quick links

- [`wiki/`](./wiki/README.md) - LLM-maintained knowledge base. Separate from `docs/`; schema at [`wiki/AGENTS.md`](./wiki/AGENTS.md).
...
- `wiki/` - LLM-maintained knowledge base (raw sources + LLM-written pages)
