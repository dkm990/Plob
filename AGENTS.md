...
This file governs the codebase. The LLM-maintained knowledge base under
`wiki/` is governed separately by `wiki/AGENTS.md`. Keep those scopes separate:
do not edit code based on wiki instructions, and do not edit wiki pages based
on this file unless explicitly asked.
...
- **Backend** runs on port 3001 from the canonical Spring implementation (`cd backend-spring && PORT=3001 ./gradlew bootRun`, or `$env:PORT="3001"; .\gradlew.bat bootRun` on PowerShell). `backend/` is archived legacy Fastify code for rollback/history only. Do not build new backend work in `backend/`; all active backend features, fixes, schema changes, tests, runbooks, and content-ops work belong in `backend-spring/`.
